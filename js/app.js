 ...
 */
    ...
    (() => {
        ...
        if (telInputs.length > 0) flsModules.inputmask = Inputmask({
        ...
        function initSliders() {
            if (document.querySelector(".services__slider")) new swiper_core_Swiper(".services__slider", {
                ...
                centeredSlides: true,
                autoHeight: true,
                initialSlide: Math.floor(document.querySelectorAll(".services__slide").length / 2),
                speed: 800,
                coverflowEffect: {
                    rotate: 0,
                    stretch: 100,
...
***************************************************************************** */
        ...
        if (galleries.length) {
            ...
            galleries.forEach((gallery => {
                const elements = gallery.querySelectorAll(".interior__image img");
                const dynamicGalleryItems = Array.from(elements).map((img => ({
                    src: img.src,
                    thumb: img.src,
                    alt: img.alt
                })));
                if (dynamicGalleryItems.length > 0) {
                    ...
                    elements.forEach(((img, imgIndex) => {
                        img.addEventListener("click", (e => {
                            e.preventDefault();
                            galleryInstance.openGallery(imgIndex);
                        }));
                    }));
                    galleryItems.push({
                        gallery,
        ...
        const menuLinks = document.querySelectorAll(".menu li._scroll a");
        if (menuLinks.length > 0) menuLinks.forEach((link => {
            link.dataset.goto = "#" + link.href.split("#")[1];
        }));
        ...
        if (canvas) {
            ...
            const circleDataBlock = document.getElementById("circle-data");
            const texts = [];
            for (let i = 1; i <= 15; i++) {
                const text = circleDataBlock.dataset[`text-${i}`];
                if (text) texts.push(text);
            }
            const centerText = circleDataBlock.dataset.textCenter;
            const centerCircle = {
                x: canvas.width / 2,
                y: canvas.height / 2,
                radius: 100,
            ...
            const circles = texts.map(((text, index) => {
                let circle;
                ...
                while (!isValidPosition) {
                    ...
                    if (distance >= circle.radius + centerCircle.radius && circle.x - circle.radius > 0 && circle.x + circle.radius < canvas.width && circle.y - circle.radius > 0 && circle.y + circle.radius < canvas.height) isValidPosition = true;
                }
                return circle;
            }));
            ...
            function drawCenterCircle() {
                ...
                ctx.fill();
                ctx.closePath();
                ctx.fillStyle = "#111b1b";
                ctx.font = "20px 'Arial'";
                ctx.textAlign = "center";
                ctx.textBaseline = "middle";
                ctx.fillText(centerCircle.text, centerCircle.x, centerCircle.y);
            }
        ...
        if (equipments.length > 0) equipments.forEach((header => {
            const body = header.querySelector(".equipment__content");
